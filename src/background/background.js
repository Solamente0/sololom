...
async function fetchLLMResponse(data) {
  ...
  if (!apiKey) {
    throw new Error('API key not configured for this model. Please check settings.');
  }
...
async function saveConversation(conversation) {
  ...
  const { conversations, chatSettings } = await chrome.storage.sync.get(['conversations', 'chatSettings']);
  const maxHistory = chatSettings.maxHistoryItems || 100;
  
  // Add new conversation and limit to max history size
  const updatedConversations = [conversation, ...conversations].slice(0, maxHistory);
