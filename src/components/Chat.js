...
export default class Chat {
  ...
  async init() {
    try {
      ...
      if (this.options.conversationId) {
        const conversation = await StorageService.getConversationById(
        ...
        if (conversation) {
          ...
          this.chatSettings = {
            ...
            temperature:
              conversation.temperature || this.chatSettings.temperature,
            maxTokens: conversation.maxTokens || this.chatSettings.maxTokens,
            systemPrompt:
              conversation.systemPrompt || this.chatSettings.systemPrompt,
      ...
      if (
        this.conversation.filter((msg) => msg.role !== "system").length === 0
      ) {
        this.showEmptyState();
      }
    } catch (error) {
  ...
  async sendMessage() {
    ...
    try {
      ...
      const assistantMessage = ApiService.extractContentFromResponse(
        response,
        provider
      );
      ...
      if (assistantMessage) {
        ...
        if (
          ...
          this.globalSettings.saveConversations !== false
        ) {
  ...
  clearChat() {
    // Keep system message if present
    const systemMessage = this.conversation.find(
      (msg) => msg.role === "system"
    );
  ...
  exportChat(format = "markdown") {
    let content = "";
    let filename = `${(this.chatSettings.title || "conversation")
      .replace(/[^a-z0-9]/gi, "_")
