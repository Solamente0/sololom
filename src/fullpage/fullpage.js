...
document.addEventListener("DOMContentLoaded", async () => {
  ...
  document.body.setAttribute(
    "data-font-size",
    globalSettings.fontSize || "medium"
  );
  document.body.setAttribute(
    "data-compact",
    globalSettings.compactMode ? "true" : "false"
  );
  ...
  if (!apiKey) {
    displayErrorMessage(
      "API key not set for the selected model. Please configure in settings."
    );
  }
...
modelSelect.addEventListener("change", () => {
  ...
  if (!apiKey) {
    displayErrorMessage(
      "API key not set for selected model. Please configure in settings."
    );
  }
...
function populateChatSettingsForm() {
  ...
  systemPromptInput.value =
    chatSettings.systemPrompt || "You are a helpful assistant.";
}
...
function exportChat() {
  ...
  const format = prompt(
    `Choose export format (${exportOptions.join(", ")}):`,
    "Markdown"
  );

  if (
    !format ||
    !exportOptions.map((o) => o.toLowerCase()).includes(format.toLowerCase())
  ) {
    return;
  }

  let content = "";
  let filename = `${chatSettings.title
    .replace(/[^a-z0-9]/gi, "_")
