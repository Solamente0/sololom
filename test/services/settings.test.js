...
describe('Settings Service', () => {
  ...
  describe('updateGlobalSettings', () => {
    it('should merge new settings with existing settings', async () => {
      ...
      const updatedSettings = await SettingsService.updateGlobalSettings(newSettings);
    ...
    it('should notify subscribers when settings are updated', async () => {
      ...
      const unsubscribe = SettingsService.subscribeToSettingsChanges('global', subscriber);
  ...
  describe('updateChatSettings', () => {
    it('should merge new settings with existing settings', async () => {
      ...
      const updatedSettings = await SettingsService.updateChatSettings(newSettings);
    ...
    it('should notify subscribers when settings are updated', async () => {
      ...
      const unsubscribe = SettingsService.subscribeToSettingsChanges('chat', subscriber);
  ...
  describe('resetGlobalSettings', () => {
    it('should reset global settings to defaults', async () => {
      ...
      expect(StorageService.saveGlobalSettings).toHaveBeenCalledWith(defaultSettings);
  ...
  describe('resetChatSettings', () => {
    it('should reset chat settings to defaults', async () => {
      ...
      expect(StorageService.saveChatSettings).toHaveBeenCalledWith(defaultSettings);
      expect(resetSettings).toEqual(defaultSettings);
    });
  });
  ...
  describe('subscribeToSettingsChanges', () => {
    ...
    it('should add and remove event handlers', async () => {
      ...
      // Subscribe to global settings changes
      const unsubscribe1 = SettingsService.subscribeToSettingsChanges('global', subscriber1);
      const unsubscribe2 = SettingsService.subscribeToSettingsChanges('global', subscriber2);
  ...
  describe('getGlobalSetting / setGlobalSetting', () => {
    it('should get a specific global setting value', async () => {
      ...
      const nonExistent = await SettingsService.getGlobalSetting('nonExistent', 'default');
      expect(nonExistent).toBe('default');
    });
    
    it('should handle nested keys', async () => {
      const apiKey = await SettingsService.getGlobalSetting('apiKeys.openai', '');
      expect(apiKey).toBe('test-key');
      
      const nonExistentNested = await SettingsService.getGlobalSetting('apiKeys.nonExistent', 'default');
  ...
  describe('getChatSetting / setChatSetting', () => {
    it('should get a specific chat setting value', async () => {
      const temperature = await SettingsService.getChatSetting('temperature', 0.5);
      expect(temperature).toBe(0.7);
      
      const nonExistent = await SettingsService.getChatSetting('nonExistent', 'default');
      expect(nonExistent).toBe('default');
    });
  ...
  describe('UI theme functions', () => {
    ...
    it('should apply font size to the document body', () => {
      ...
      expect(document.body.setAttribute).toHaveBeenCalledWith('data-font-size', 'large');
    });
    ...
    it('should apply compact mode to the document body', () => {
      ...
      expect(document.body.setAttribute).toHaveBeenCalledWith('data-compact', 'true');
    });
