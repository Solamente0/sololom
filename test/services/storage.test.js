...
describe('Storage Service', () => {
  ...
  describe('saveGlobalSettings', () => {
    ...
    it('should handle errors when saving global settings', async () => {
      ...
      await expect(StorageService.saveGlobalSettings(settings)).rejects.toThrow('Test error');
    });
  });
  ...
  describe('saveConversation', () => {
    ...
    it('should limit the number of conversations based on global settings', async () => {
      ...
      const savedConversations = chrome.storage.sync.set.mock.calls[0][0].conversations;
  ...
  describe('exportData and importData', () => {
    ...
    describe('exportData', () => {
      ...
      it('should include real API keys if includeApiKeys option is true', async () => {
        const jsonData = await StorageService.exportData({ includeApiKeys: true });
        const data = JSON.parse(jsonData);
    ...
    describe('importData', () => {
      it('should import data without overwriting existing data by default', async () => {
        ...
        const setCall = chrome.storage.sync.set.mock.calls.find(call => call[0].conversations);
      ...
      it('should overwrite existing data if overwrite option is true', async () => {
        ...
        await StorageService.importData(JSON.stringify(importData), { overwrite: true });
      ...
      it('should not import placeholder API keys', async () => {
        ...
        await StorageService.importData(JSON.stringify(importData), { importApiKeys: true });
      ...
      it('should reject with an error if the imported data format is invalid', async () => {
        ...
        await expect(StorageService.importData(invalidData)).rejects.toThrow('Invalid import data format');
      });
    });
  });
});
