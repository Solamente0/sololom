...
describe('errorHandler utility functions', () => {
  describe('handleApiError', () => {
    it('should return user-friendly messages for OpenAI API errors', () => {
      const insufficientQuotaError = new Error('OpenAI API Error: insufficient_quota');
      expect(errorHandler.handleApiError(insufficientQuotaError))
        .toBe('Your OpenAI API key has insufficient quota. Please check your billing status.');
      
      const invalidApiKeyError = new Error('OpenAI API Error: invalid_api_key');
      expect(errorHandler.handleApiError(invalidApiKeyError))
        .toBe('Invalid OpenAI API key. Please check your settings.');
      
      const rateLimitError = new Error('OpenAI API Error: rate_limit_exceeded');
      expect(errorHandler.handleApiError(rateLimitError))
        .toBe('OpenAI rate limit exceeded. Please try again later.');
    });
    
    it('should return user-friendly messages for Anthropic API errors', () => {
      const invalidApiKeyError = new Error('Anthropic API Error: invalid_api_key');
      expect(errorHandler.handleApiError(invalidApiKeyError))
        .toBe('Invalid Anthropic API key. Please check your settings.');
      
      const permissionError = new Error('Anthropic API Error: permission_error');
      expect(errorHandler.handleApiError(permissionError))
        .toBe('Your Anthropic API key does not have permission to use this model.');
    });
    
    it('should return user-friendly messages for Mistral API errors', () => {
      const invalidApiKeyError = new Error('Mistral API Error: invalid_api_key');
      expect(errorHandler.handleApiError(invalidApiKeyError))
        .toBe('Invalid Mistral API key. Please check your settings.');
      
      const rateLimitError = new Error('Mistral API Error: rate_limit_exceeded');
      expect(errorHandler.handleApiError(rateLimitError))
        .toBe('Mistral rate limit exceeded. Please try again later.');
    });
    
    it('should handle network errors', () => {
      const networkError = new Error('Failed to fetch');
      expect(errorHandler.handleApiError(networkError))
    ...
    it('should return the original message for unrecognized API errors', () => {
      ...
      expect(errorHandler.handleApiError(unknownApiError))
        .toBe('OpenAI API Error: unknown_error');
    });
    
    it('should return a generic message for errors without a message', () => {
      const emptyError = new Error();
      expect(errorHandler.handleApiError(emptyError))
  ...
  describe('withErrorHandling', () => {
    ...
    it('should use custom error handler if provided', async () => {
      const error = new Error('Test error');
      const mockFn = jest.fn().mockRejectedValue(error);
      const customErrorHandler = jest.fn().mockReturnValue('custom error result');
      
      const wrappedFn = errorHandler.withErrorHandling(mockFn, customErrorHandler);
  ...
  describe('reportError', () => {
    ...
    it('should create a basic toast if showToast is not available', () => {
      ...
      const message = 'Test error message';
      errorHandler.reportError(message, 'error', { toast: true, console: false });
