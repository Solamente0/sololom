...
describe('helpers utility functions', () => {
  ...
  describe('formatDate', () => {
    ...
    it('should format date in different formats based on options', () => {
      expect(helpers.formatDate(testDate, { format: 'long' })).toBe('6/15/2023');
      expect(helpers.formatDate(testDate, { format: 'time' })).toBe('12:30 PM');
      expect(helpers.formatDate(testDate, { format: 'datetime' })).toBe('6/15/2023 12:30 PM');
    });
    ...
    it('should format relative dates when relative option is true', () => {
      ...
      expect(helpers.formatDate(testDate, { relative: true })).toBe('2 hours ago');
      ...
      expect(helpers.formatDate(fiveMinutesAgo, { relative: true })).toBe('5 minutes ago');
      
      const oneDayAgo = new Date(now - 24 * 60 * 60 * 1000);
      expect(helpers.formatDate(oneDayAgo, { relative: true })).toBe('1 day ago');
  ...
  describe('truncateString', () => {
    it('should truncate a string if it exceeds maxLength', () => {
      ...
      expect(helpers.truncateString(longString, 20)).toBe('This is a long st...');
    });
  ...
  describe('getModelDisplayName', () => {
    it('should return a display name for known models', () => {
      expect(helpers.getModelDisplayName('gpt-3.5-turbo')).toBe('GPT-3.5 Turbo');
      expect(helpers.getModelDisplayName('claude-3-opus')).toBe('Claude 3 Opus');
    });
