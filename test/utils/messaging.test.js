...
describe('messaging utility functions', () => {
  describe('sendToBackground', () => {
    ...
    it('should reject the promise if there is a runtime error', async () => {
      ...
      await expect(messaging.sendToBackground(message)).rejects.toThrow('Test error');
    });
    ...
    it('should reject the promise if sending the message throws an error', async () => {
      ...
      await expect(messaging.sendToBackground(message)).rejects.toThrow('Send error');
    });
  });
  ...
  describe('sendToTab', () => {
    ...
    it('should reject the promise if there is a runtime error', async () => {
      ...
      await expect(messaging.sendToTab(tabId, message)).rejects.toThrow('Test error');
    });
    ...
    it('should reject the promise if sending the message throws an error', async () => {
      ...
      await expect(messaging.sendToTab(tabId, message)).rejects.toThrow('Send error');
    });
  });
  ...
  describe('setupMessageListener', () => {
    ...
    it('should call the appropriate handler based on message action', () => {
      ...
      const result1 = listener({ action: 'action1', data: 'test' }, {}, sendResponse);
      const result2 = listener({ action: 'action2', data: 'test' }, {}, sendResponse);
      const result3 = listener({ action: 'unknown', data: 'test' }, {}, sendResponse);
      
      expect(handlers.action1).toHaveBeenCalledWith({ action: 'action1', data: 'test' }, {});
      expect(handlers.action2).toHaveBeenCalledWith({ action: 'action2', data: 'test' }, {});
